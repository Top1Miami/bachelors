...
def methodology_test(html, X, y, good_features, number_of_test, scores):
    ...
    for feature_train, feature_test in feature_split.split(X.T, feature_marks):
        ...
        for sample_train, sample_test in sample_split.split(X, y):
            ...
            scores.append([rec_prec, prec_prec, melif_prec.best_percentage, 'точность'])
            rec_f1, prec_f1 = score_test_2phase(feat_f1)
            scores.append([rec_f1, prec_f1, melif_f1.best_percentage, 'ф1 мера на признаках'])
            rec_2phase, prec_2phase = score_test_2phase(feat_2phase)
            scores.append([rec_2phase, prec_2phase, melif_2phase.best_percentage, 'двух-фазная мера'])
...
for number in range(1, 7):
    with open(str(number) + 'TablesPlots/shuffled.csv', 'r') as fd: # open each file 
        ...
        directory_name = str(number) + 'TablesPlots/subsamples_melif_meta' # generate the directory for storing results
        subsamples = create_subsamples(directory_name, X, y, subsample_size, 10)
        print('subsamples', len(subsamples))
        
        number_of_test = 1
        scores = []
        html = open(str(number) + 'TablesPlots/features_percentages_grid.html', 'w')
        ...
        for sub_x, sub_y in subsamples:
            print('new subset')
            number_of_test = methodology_test(html, sub_x, sub_y, good_features[number - 1], number_of_test, scores)
        __write_post(html)
        ...
        sns.lineplot(x="процент", y="полнота",
                 hue="функция потерь",
                 data=df)
        plt.savefig(str(number) + 'TablesPlots/rec_percent_grid.png')
        plt.close()
        df = pd.DataFrame(data=scores, index=range(len(scores)), columns=["полнота", "точность", "процент", "функция потерь"])
        sns.lineplot(x="процент", y="точность",
                 hue="функция потерь",
                 data=df)
        plt.savefig(str(number) + 'TablesPlots/prec_percent_grid.png')
        plt.close()
