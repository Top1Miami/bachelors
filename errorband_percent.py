...
from ITMO_FS.ensembles.measure_based.MelifLossFMeta import MelifLossFMeta
from ITMO_FS.ensembles.measure_based.Melif2PhaseMeta import Melif2PhaseMeta
...
starting_list = [0.1, 0.3, 0.5, 0.7, 1.0]
...
def methodology_test(html, X, y, good_features, number_of_test, scores):
    ...
    for feature_train, feature_test in feature_split.split(X.T, feature_marks):
        ...
        for sample_train, sample_test in sample_split.split(X, y):
            ...
            melif_prec = MelifLossFMeta(filters, score_train_prec)
            melif_prec.fit(X_ftrain[sample_train], y[sample_train], delta=delta, points=ParameterGrid(param_grid))
            melif_prec.run()
            feat_prec = melif_prec.transform(X_ftest[sample_train], y[sample_train], score_test_prec)
            ...
            melif_f1 = MelifLossFMeta(filters, score_train_f1)
            melif_f1.fit(X_ftrain[sample_train], y[sample_train], delta=delta, points=ParameterGrid(param_grid))
            melif_f1.run()
            feat_f1 = melif_f1.transform(X_ftest[sample_train], y[sample_train], score_test_f1)
            ...
            melif_2phase = Melif2PhaseMeta(filters, score_train_2phase)
            melif_2phase.fit(X_ftrain[sample_train], y[sample_train], delta=delta, points=ParameterGrid(param_grid))
            melif_2phase.run()
            feat_2phase = melif_2phase.transform(X_ftest[sample_train], y[sample_train], score_test_2phase)
            ...
            goods = [sel_prec, sel_f1, sel_2phase, sel_m]
            best_percentages = [melif_prec.best_percentage, melif_f1.best_percentage, melif_2phase.best_percentage]
            best_points = [melif_prec.best_point, melif_f1.best_point, melif_2phase.best_point, melif.best_point]
            scores_html = [score_test_2phase(feat_prec), score_test_2phase(feat_f1), score_test_2phase(feat_2phase), score_test_2phase(feat_m)]
            ...
            scores.append([rec_prec, prec_prec, melif_prec.best_percentage, 'precision'])
            rec_f1, prec_f1 = score_test_2phase(feat_f1)
            scores.append([rec_f1, prec_f1, melif_f1.best_percentage, 'f1_feature'])
            rec_2phase, prec_2phase = score_test_2phase(feat_2phase)
            scores.append([rec_2phase, prec_2phase, melif_2phase.best_percentage, '2phase'])
...
with open('2TablesPlots/shuffled.csv', 'r') as fd: # open each file 
    ...
    # good_features = np.array([1, 4, 5, 6, 8, 9, 73, 13, 14, 16, 17, 18, 19, 24, 25, 88, 89, 90, 29, 94, 98, 101, 102, 103, 104, 105, 106, 45, 46]) # dataset 6 su measure
    directory_name = '2TablesPlots/subsamples_melif' # generate the directory for storing results
    subsamples = create_subsamples(directory_name, X, y, subsample_size, 5)
    print('subsamples', len(subsamples))
    number_of_test = 1
    scores = []
    html = open('2TablesPlots/features_percentages.html', 'w')
    ...
    "good features selected by MeLiF with precision score loss", "best filter weight vector by MeLiF with precision score loss", "best percentage by Melif with precision score loss", "recall, precision score on MeLiF with precision score loss", 
    "good features selected by MeLiF with f1 score feature loss", "best filter weight vector by MeLiF with f1 score feature loss", "best percentage by Melif with f1 score feature loss ", "recall, precision score on MeLiF with f1 score feature loss", 
    "good features selected by MeLiF with 2phase score loss", "best filter weight vector by MeLiF with 2phase score loss", "best percentage by Melif with 2phase score loss", "recall precision score on MeLiF with 2phase score loss", 
    ...
    for sub_x, sub_y in subsamples:
        print('new subset')
        number_of_test = methodology_test(html, sub_x, sub_y, good_features, number_of_test, scores)
    __write_post(html)
    df = pd.DataFrame(data=scores, index=range(len(scores)), columns=["recall", "precision", "percentage", "loss"])
    sns.lineplot(x="percentage", y="recall",
             hue="loss",
             data=df)
    plt.savefig('2TablesPlots/rec_percent.png')
    plt.close()
    df = pd.DataFrame(data=scores, index=range(len(scores)), columns=["recall", "precision", "percentage", "loss"])
    sns.lineplot(x="percentage", y="precision",
             hue="loss",
             data=df)
    plt.savefig('2TablesPlots/prec_percent.png')
