...
def methodology_test(X, y, good_features, number_of_test, scores, k):
    ...
    for feature_train, feature_test in feature_split.split(X.T, feature_marks):
        ...
        for sample_train, sample_test in sample_split.split(X, y):
            ...
            scores.append([rec_prec, prec_prec, k, 'precision'])
            ...
            scores.append([rec_m, prec_m, k, 'f1_object'])
...
for number in range(1, 7):
    ...
    with open(str(number) + 'TablesPlots/shuffled.csv', 'r') as fd: # open each file 
        ...
        df1 = pd.DataFrame(data=scores, index=range(len(scores)), columns=["recall", "precision", "cutting_rule", "loss"])
        ...
        df2 = pd.DataFrame(data=scores, index=range(len(scores)), columns=["recall", "precision", "cutting_rule", "loss"])
