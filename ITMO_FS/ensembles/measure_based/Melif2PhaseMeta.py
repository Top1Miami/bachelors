...
class Melif2PhaseMeta:
    ...
    def __init__(self, filters, score=None):  # TODO scorer name
        ...
        self.best_rec = 0 # best accumulated recall score during the work of Melif
        self.best_prec = 0 # best accumulated precision score during the wrok of Melif
    ...
    def fit(self, X, y, delta=0.5, feature_names=None, points=None):
        """
        TODO comments
        :param X:
        :param y:
    ...
    def __search(self, points, features): # search for feature weights
        ...
        for cur_point in points:
            ...
            for percentage in range(80, 100):
                ...
                rec_score, prec_score = self.__score(keys)
                if  prec_score > self.best_prec: # if current score is better
                    self.best_prec = prec_score # save current score as best
                    self.best_rec = rec_score
                ...
                elif prec_score == self.best_prec and rec_score > self.best_rec:
                    self.best_rec = rec_score
                    self.best_point = cur_point # save current filter weight vector as best
                    self.best_f = new_features  # save selected features as best
                    self.best_percentage = percentage
    ...
    def transform(self, X, y, score_func, feature_names=None): # select features according to current filter weight vector
        ...
        # keys = self.select_by_percentage(n, self.best_percentage) # select the features according to the composed scores
        ...
        for percentage in range(80, 100):
            keys = self.select_by_percentage(n, percentage) # select the features according to the composed scores
            new_features = {i: features[i] for i in keys} # dicitionary selected feature number -> its original name
            rec_score, prec_score = score_func(keys)
            if prec_score > best_prec_score:
                best_prec_score = prec_score
                best_rec_score = rec_score
                best_keys = keys
                best_percentage = percentage
            elif prec_score == best_prec_score and rec_score > best_rec_score:
                best_prec_score = prec_score
                best_rec_score = rec_score
                best_keys = keys
                best_percentage = percentage
        ...
        # new_features = {i: nu[i] for i in keys} # dicitionary selected feature number -> its original name
        # return list(new_features.keys())
        new_features = {i: nu[i] for i in best_keys} # dicitionary selected feature number -> its original name  
        return list(new_features.keys())
