...
def __complementarity(x_i, x_j, y):
...
def __chained_information(x_i, x_j, y):
    return mutual_information(x_i, y) + mutual_information(x_j, y) + __complementarity(x_i, x_j, y)
    ...
    def run(self, X, y):
        ...
        for i in range(self.n_features):
            for j in range(self.n_features):
                ...
                if entropy_pair != 0.:
                    self._edges[i][j] = __chained_information(X[:, i], X[:, j], y) / entropy_pair
